...
1.  **Initialize Project:** Setup `requirements.txt` / `pyproject.toml` (e.g., `requests`, `google-generativeai`, `pydantic`). We might consider using `langchain` or `langgraph` if the agent logic gets more complex, though standard python is fine for MVP.
2.  **API Client Draft:** Write the `OpenAlexClient` wrapper to handle fetching and parsing raw data into our Metadata schema.
3.  **LLM Evaluator Draft:** Write the structured prompt (using Gemini's `response_schema`/`response_mime_type="application/json"` if utilizing `google-generativeai`) that takes a user query and paper metadata, returning a standardized relevance assessment.
4.  **Loop Logic Integration:** Assemble the `AgentOrchestrator` while putting strict safeguards on the `while` loop.
